...
void obtenerMatriz(const char* path, int (&distancias)[21][21]) {
	ifstream file;
...
void obtenerMatrizSolucion(string domicilios[15], int (&matriz)[21][21]) {
...
void multiplicarMatrices(int a[21][21], int b[21][21], int (&c)[21]) {
...
void intercambiosCruzados(string solActual[3][15], string solNueva[3][15], int distancias[21][21],
		int tiempos[21][21]) {
	cout << "-Iniciando el proceso de intercambios entre combis " << endl << endl << "Recorrido inicial:" << endl
			<< endl;
				...
				for (int j = i + 1; j < 3; j++) {
					...
					for (int l = 0; l < 15; l++) {
						...
						if (!solNueva[j][l].empty() && !solNueva[j][l + 1].empty()) {
							...
							if (tiempoPart01 <= tiempoPart0 && tiempoPart10 <= tiempoPart1 && distPart01 <= distPart0
									&& distPart10 <= distPart1) {
...
void kIntercambios(string solActual[3][15], string solNueva[3][15], int distancias[21][21], int tiempos[21][21]) {
	...
	for (int i = 0; i < 3; i++) {
		...
		cout << endl;
		cout << "Inicial " << "Combi " << i + 1 << "\t\t Tiempo \t" << t0 << " minutos \t" << "Distancia \t" << d0
				<< " kilómetros " << endl;
		...
		cout << "Final " << "Combi " << i + 1 << "\t\t Tiempo \t" << t0 << " minutos \t" << "Distancia \t" << d0
				<< " kilómetros" << endl;
		cout << endl;
	}
